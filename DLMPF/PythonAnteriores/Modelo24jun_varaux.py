...
def main():
    ...
    w = model.integer_var_cube(keys1=CONJ_P_itens, keys2=CONJ_L_lin, keys3=CONJ_T_per, name='Parcial[%s,%s,%s]')
    z = model.binary_var_cube(keys1=CONJ_P_itens, keys2=CONJ_L_lin, keys3=range(1,T), name='Z[%s,%s,%s]')
    aux_var = model.continuous_var_matrix(keys1=CONJ_L_lin, keys2=range(1,T), name = 'aux')
    ...
    #        model.add_constraint_(model.sum(p[i][k] * (Q * x[i,k,t] + d_tam_residual[i] * y[i,k,t] + w[i,k,t-1] - w[i,k,t]) for i in CONJ_P_itens if i in p and k in p[i]) <= Ch[k][t], ctname='Restricao_11')
                ...
                #model.add_constraint_(model.sum(p[i][k] * (Q * x[i,k,t] + d_tam_residual[i] * y[i,k,t] + w[i,k,t-1] - w[i,k,t]) for i in CONJ_P_itens if i in p and k in p[i]) <= Ch[k][t] + aux_var[k,t], ctname='Restricao_11')
