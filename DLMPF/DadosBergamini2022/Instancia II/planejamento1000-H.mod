...
var soluc;
var fobj;
#var excesso{recursos, periodos} >= 0;

# FUNCAO OBJETIVO

minimize tempo: sum{i in modelos, k in linhas, t in periodos: t>0} ((tprocessamento[i,k])*Q*x[i,k,t] + (tprocessamento[i,k])*(demanda[i] mod Q)*y[i,k,t] + 1000 * z[i,k,t]); 
#+ sum{r in recursos, t in periodos: t>0} 1000*excesso[r,t];
...
display tempo, restr_tempoproducao, restr_capacidadediaria;
