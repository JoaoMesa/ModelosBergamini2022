...
#a sobra para o dia seguinte tem tamanho m�ximo de 240
...
# Relaciona a variavel de tamanho do fracionamento (parcial) com a ocorr�ncia de fracionamento (z)
# A vari�vel parcial pode ser positiva apenas se houver fracionamento no periodo, e est� limitada ao tamanho do lote menos o lote minimo
restr_parcial3a{i in modelos, k in linhas, t in periodos: t>0}: 
	parcial[i,k,t] <= (Q-q) * z[i,k,t];

# A variavel parcial deve respeitar o lote minimo de q unidades, se este for fra��o do ultimo lote do dia
restr_parcial3b{i in modelos, k in linhas, t in periodos: t>0}: 
	q * z[i,k,t] <= parcial[i,k,t];

# Limita o fracionamento de lotes em cada linha e em cada periodo em no m�ximo 1.
...
# A soma de todos os modelos, em todas as linhas e em todos os periodos de um grupo de recursos r n�o pode exceder a capacidade do mesmo
...
# O somatorio de todos os modelos, em todas as linhas n�o deve exceder a capacidade di�ria dos recursos. 
