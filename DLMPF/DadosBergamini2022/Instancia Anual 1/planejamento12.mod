...
var soluc;
var fobj >= 0;
var excessoC{recursos, periodos} >= 0;

# FUNCAO OBJETIVO

minimize tempo: sum{k in linhas, t in periodos: t>0} excessoC[k,t] ;

#FUNCAO OBJETIVO ORIGINAL

restr_fobj: 
	fobj = sum{i in modelos, k in linhas, t in periodos: t>0} ((tprocessamento[i,k])*Q*x[i,k,t] + (tprocessamento[i,k])*(demanda[i] mod Q)*y[i,k,t] + z[i,k,t]);
...
restr_tempoproducao{k in linhas, t in periodos: t>0}: 
	sum {i in modelos} tprocessamento[i,k]*(Q*x[i,k,t] + (demanda[i] mod Q)*y[i,k,t] + parcial[i,k,t-1] - parcial[i,k,t]) <= C[k,t] + excessoC[k,t];
