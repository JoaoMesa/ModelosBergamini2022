...
def main():
    ...
    read_capacidade = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/capacidade.csv")
    read_demanda = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/demanda.csv")
    read_itens = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/modelos.csv")
    read_mapa_modelo_linha = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/mapalinhamodelo.csv")
    read_mapa_linha_recurso = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/mapalinharecurso.csv")
    read_mapa_modelo_recurso = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/mapamodelorecurso.csv")
    read_C = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/C.csv")
    read_t_processamento = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/tprocessamento.csv")
    read_capacidade_diaria = pd.read_csv("../DLMPF/DadosBergamini2022/Instancia I/capacidadediaria.csv")
    ...
    for periodo in range(1, 6):
        indices_binario.append(periodo)
        for indice in indices_binario:
            for i in CONJ_P_itens:
                for k in CONJ_L_lin:
                    x[i, k, indice].set_vartype(model.integer_vartype)
                    w[i, k, indice].set_vartype(model.integer_vartype)
                    y[i, k, indice].set_vartype(model.binary_vartype)
                    z[i, k, indice].set_vartype(model.binary_vartype)
                    
        nova_solucao = model.solve()

        if nova_solucao:
            print("Períodos Binárias/Inteiros: ", indices_binario)
            print("Períodos Fixados: ", indices_binario[:-1])
            print(nova_solucao)
        else:
        ...
        for i in CONJ_P_itens:
            for k in CONJ_L_lin:
                x[i, k, periodo].lb = x[i, k, periodo].ub = nova_solucao.get_value(x[i,k,periodo])
                y[i, k, periodo].lb = y[i, k, periodo].ub = nova_solucao.get_value(y[i,k,periodo])
                w[i, k, periodo].lb = w[i, k, periodo].ub = nova_solucao.get_value(w[i,k,periodo])
                z[i, k, periodo].lb = z[i, k, periodo].lb = nova_solucao.get_value(z[i,k,periodo])
            
    solucaoRelaxFix = nova_solucao
    
    print(solucaoRelaxFix)
